...
### Kiểm tra RBAC ở từng action, không gắn chung ở route
Mỗi hàm controller (index, create, edit, delete, changeStatus, changeMulti, restore, deletePermanently...) tự kiểm tra `req.role.permissions.includes("module_action")` riêng. Việc này cho phép cùng một route pattern (`/tasks`) nhưng action GET và POST có thể cần quyền khác nhau (`tasks_view` vs `tasks_create`), linh hoạt hơn middleware gắn chung theo route.

### Cron job dọn `updatedBy` để tránh phình document
`updatedBy` là mảng — mỗi lần sửa push thêm một entry. Nếu để mãi, document sẽ phình to theo thời gian (mỗi lần sửa = +1 object). Cron job dùng MongoDB `$pull` để xóa hàng loạt các entry cũ hơn 1 tháng vào 2h sáng mỗi ngày, giữ lịch sử gần nhất mà không cần logic phức tạp.

### TTL Index cho OTP thay vì dùng cron
Thay vì viết cron job để dọn OTP hết hạn, dùng MongoDB TTL Index: `forgotPasswordSchema.index({ expireAt: 1 }, { expireAfterSeconds: 120 })`. MongoDB tự xóa document sau 120 giây — đơn giản, không cần code thêm, không lo sót.

### Soft delete + Audit trail
Tất cả entity quan trọng đều có `deleted: Boolean` và `createdBy`/`updatedBy`/`deletedBy`. Mọi query bình thường đều filter `{ deleted: false }`. Dữ liệu "xóa" vẫn còn trong DB và có thể khôi phục. Đây là pattern phổ biến trong hệ thống thực tế để tránh mất dữ liệu do thao tác nhầm.

### Hai namespace JWT độc lập cho admin và client
Admin (`Account`) và client (`User`) đều dùng cùng `SECRET_KEY` để ký token, nhưng khi verify đều query đúng collection tương ứng (admin middleware query `Account`, client middleware query `User`). Điều này đảm bảo token của client không thể dùng để vào admin API và ngược lại, dù cùng chung key ký.
